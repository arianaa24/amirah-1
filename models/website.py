...
class Website(models.Model):
    ...
    def existencias(self, producto):
        existencias = ''
        mensaje = ''
        cantidades = self.env['stock.quant'].sudo().search([('quantity','>',0),('product_id.id','=',producto),('location_id.productos_sitio_web','=',True)], limit=2)
    ...
    def producto_agotado(self,productos, attribute_value=None):
        ...
        cantidades = self.env['stock.quant'].sudo().search([('quantity','>',0),('product_id','in',productos.ids), ('location_id.productos_sitio_web','=',True)])
        cantidad = 0
        for quant in cantidades:
            if attribute_value:
